...
量化投资策略设计与分析的第一次作业是基于逐笔数据构造39个高频因子。我对高频因子的构造经验比较少，完成这个作业后的一些经验：

1. 高频因子的数据格式是比较标准化的，但也要注意细节：例如空缺时间的填补等。
2. 构造因子的过程本质上是数据处理的过程，常用的方法有：`groupby`、`resample`、`to_datetime`等。如果是非常大的数据集，应当用 `numpy` 等更快速的科学计算包，或者用`C++`。

因为最开始希望减少函数之间的依赖，所以因子函数中尽量少用嵌套函数的写法。但这样做的一个弊端就是代码很长，有非常多重复的代码，最好还需要重构一下。

目前的代码只是完成作业的粗糙版本，构造因子的一些细节并没有达到完美，例如日期时间格式的空缺值、非交易时段的数据处理等，都需要进一步检查。

## 需求

用提供的某支证券为期不超过一周的高频数据复制Table A2中39种指标。
...
order = pd.read_csv(
    "./data/order_stkhf202101_000001sz.csv",
    dtype={
        "Exchflg": "int",
        "Code": "string",
        "Code_Mkt": "string",
...
for i in tqdm(range(1, 39 + 1)):
    if i <= 8 or 24 <= i <= 27:
        # 使用 order 计算因子
        Ai = eval("calculate_A" + str(i))(order)
    elif 10 <= i <= 16 or i >= 28:
        # 使用 trade 计算因子
        Ai = eval("calculate_A" + str(i))(trade)
    elif 17 <= i <= 23:
        # 使用 trade 和 order 计算因子
        Ai = eval("calculate_A" + str(i))(trade, order)
    else:
        # i == 9，不可计算
        continue
    # Save factor
    Ai.to_csv("./factor/A" + str(i) + ".csv", index=False)
```

## 构造因子的函数

<details 构造因子的函数></summary>构造因子的函数</summary>
